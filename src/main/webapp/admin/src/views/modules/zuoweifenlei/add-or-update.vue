...
export default {
  data() {
	...
	zuoweifenlei : false,
      ...
      rules: {
          zuoweifenlei: [
                { required: true, message: '座位分类不能为空', trigger: 'blur' },
          ],
      }
    };
  },
  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `zuoweifenlei/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.zuoweifenleiCrossAddOrUpdateFlag = false;
                  this.parent.search();
